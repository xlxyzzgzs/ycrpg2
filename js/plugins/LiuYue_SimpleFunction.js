...
Game_System.prototype.PushZzySFFFlashPicArr = function (
    name,
    frame,
    x,
    y //压入
...
Game_Player.prototype.updateEncounterCount = function () {
    if ($gameSystem.GetIsZzySFFNoEncounter()) {
        return false;
    }
...
Scene_Map.prototype.CreateZzySFFFlashSpr = function (
    info //创造闪烁位图
) {
...
Zzy.SFF.SaveActor = function (actorId) {
    var actor = $gameActors.actor(actorId);
    if (!actor) {
        console.log("不存在ID为:" + actorId + "值的这个角色");
        return;
    }
    var info = {};
    info.hp = actor.hp;
...
Zzy.SFF.LoadActor = function (actorId) {
    ...
    if (!info) {
        console.log("没有通过SaveActor对角色进行提前存储");
        return;
    }
...
Zzy.SFF.CallSkillMenu = function (actorId) {
    if (!actorId) {
        console.log("执行失败:可能是因为没有输入角色ID值导致的");
        return;
    }
    var actor = $gameActors.actor(actorId);
...
Zzy.SFF.Save = function (
    saveID //这会进行存档,需要设置存档ID
) {
...
Zzy.SFF.Load = function (
    loadID //这会进行读档,需要设置存档ID
) {
    打开读档界面();
...
Zzy.SFF.SnapToLocal = function (
    path,
    fileName //截图到本地 path需要输入路径  fileName需要输入文件名
) {
    path = path ? path : "";
    ...
    if (!snapBitmap) {
        console.log("Error:(来自LiuYue_GameCore)未知错误:执行SceneManager.snap()失败");
        return false;
    }
...
Zzy.SFF.SnapToPictures = function (
    fileName //截图到pcitures文件夹中
) {
    return Zzy.SFF.SnapToLocal("img/pictures", fileName);
};

Zzy.SFF.SnapToSystem = function (
    fileName //截图到system文件夹中
) {
...
Zzy.SFF.TransferPlayer = function (
    mapId,
    x,
    y,
    dir,
...
Zzy.SFF.GetItemCount = function (
    itemId,
    varId //将读取到的道具数量存放到某个变量值
) {
...
Zzy.SFF.AddItemCountPer = function (
    itemId,
    per //对现有道具数量增加百分比的额外数值,输入10代表10%
) {
    var item = $dataItems[itemId];
    var count = Math.round($gameParty.numItems(item) * per * 0.01);
    $gameParty.gainItem(item, count);
};
var 增加道具数量百分比 = function (
    itemId,
    per //对现有道具数量增加百分比的额外数值,输入10代表10%
) {
...
Zzy.SFF.SubItemCountPer = function (
    itemId,
    per //对现有道具数量减少百分比的额外数值,范围0~100
) {
    var item = $dataItems[itemId];
    var count = Math.round($gameParty.numItems(item) * per * 0.01);
    $gameParty.gainItem(item, -count);
};
var 减少道具数量百分比 = function (
    per //对现有道具数量减少百分比的额外数值,范围0~100
) {
    Zzy.SFF.SubItemCountPer(per);
};

Zzy.SFF.GetActorLevel = function (
    actorId,
    varId //将指定的角色的等级存放到变量中,需要输入存放的变量ID值,如果角色ID输入0,则获取队长的信息
) {
...
Zzy.SFF.GetActorMaxHp = function (
    actorId,
    varId //将指定的角色的等级存放到变量中,需要输入存放的变量ID值,如果角色ID输入0,则获取队长的信息
) {
    var actor = actorId ? $gameActors.actor(actorId) : $gameParty.leader();
...
Zzy.SFF.GetActorMaxMp = function (
    actorId,
    varId //将指定的角色的等级存放到变量中,需要输入存放的变量ID值,如果角色ID输入0,则获取队长的信息
) {
    var actor = actorId ? $gameActors.actor(actorId) : $gameParty.leader();
...
Zzy.SFF.SetPlayerEncounter = function (
    enable //填写后玩家在地图中将会/不会再遇敌而触发战斗,填写true/false
) {
    enable = enable ? false : true;
    $gameSystem.SetIsZzySFFNoEncounter(enable);
};
var 设置玩家能否遇敌 = function (
    enable //填写后玩家在地图中将会/不会再遇敌而触发战斗
) {
