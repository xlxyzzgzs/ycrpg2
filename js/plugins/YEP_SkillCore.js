 ...
 * ============================================================================
 *
 * 假设你使用的所有插件均保持HP、MP、TP这个顺序，并且不会推翻默认
 * 的数值运算进程，那么这个插件也能让你将HP、MP和TP数值类型重新排列
 * 为你想要的顺序。如果你使用任何扩展插件，它们也同样会被重排。
 *
 * 请注意：如果数据库中系统页下的“在战斗中显示TP”一项没有被选择，
 * 那么第三个位置的数值类型将不被显示。
 *
 * 职业的注释:
 *   <Swap Gauge x: y>
 *   把数据类型x（1,2或3）改为y。将y替换为“HP”,“MP”或“TP”来把
 ...
 *   }
 *   </Custom Show Eval>
 *   若“visible”设为“true”，那么技能就被显示（不被隐藏）而且
 *   是可用的，只要其它条件也满足。若“visible”设为“false”，那么技能
 *   就不可使用，并且从列表中被隐藏。
 *
 * ============================================================================
 ...
 * ============================================================================
 *
 * 如果你只有很少的JavaScript的使用经验，可以用下面的注释来限制
 * 一个技能以及执行技能时使用的代码类型。
 *
 * Skill Notetags:
 *
 ...
 *    特效阶段（被插件影响）之后
 *
 * 这里有四个可被插件影响的阶段。其中两个与特效是否land无关，另外
 * 两个只当技能land才起作用。
 *
 * 技能和物品注释:
 *   <Before Eval>
...
Yanfly.Skill.Game_BattlerBase_mSC = Game_BattlerBase.prototype.meetsSkillConditions;
...
Yanfly.Skill.Game_BattlerBase_canPaySkillCost = Game_BattlerBase.prototype.canPaySkillCost;
...
Yanfly.Skill.Game_BattlerBase_paySkillCost = Game_BattlerBase.prototype.paySkillCost;
...
Yanfly.Skill.Window_SkillList_includes = Window_SkillList.prototype.includes;
