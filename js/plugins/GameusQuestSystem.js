...
Game_Party.prototype.totalQuests = function (filter) {
    ...
    if (filter === undefined || filter === "all") return this.quests.length;
    ...
    for (var i = 0; i < this.quests.length; i += 1) {
        ...
        if (q.status === filter.toLowerCase()) count += 1;
    }
    return count;
};
...
Game_Party.prototype.getQuests = function (filter) {
    // Returns a list of ALL quests
    if (filter === undefined || filter === "all") return this.quests;
    // Time to cycle through quests....(dammit, I did this exact thing in the quest list window)
    var data = [];
    for (var i = 0; i < this.quests.length; i += 1) {
        var q = $gameQuest.get(this.quests[i]);
        if (q.status === filter.toLowerCase()) data.push(q.questId);
    }
    return data;
};
...
Game_Party.prototype.hasQuests = function (quests, filter) {
    ...
    for (var i = 0; i < quests.length; i += 1) {
        if (!this.hasQuest(quests[i])) flag = false;
        if ($gameQuests.get(quests[i]).status !== filter) flag = false;
    }
    return flag;
};
...
Game_Quests.prototype.categories = function () {
    if ($dataQuests[0]) return $dataQuests[0];
    return null;
};

Game_Quests.prototype.totalQuests = function (filter) {
    // Returns a list of ALL quests
    if (filter === undefined || filter === "all") return $dataQuests.length;
    // Time to cycle through quests....(dammit, I did this exact thing in the quest list window)
    var count = 0;
...
Window_Base.prototype.sliceText = function (text, width) {
    ...
    for (var i = 0; i < words.length; i += 1) {
        ...
        if (i >= words.length - 1) result.push(current_text);
    }
    return result;
};
//---------------------------------------------------------------------------------------------
...
Window_QuestInfo.prototype.initialize = function () {
    ...
    if (this.failedImg !== "") this.failedImg = ImageManager.loadPicture(this.failedImg, 0);
    if (this.completedImg !== "") this.completedImg = ImageManager.loadPicture(this.completedImg, 0);
    this.quest = 0;
    this.offY = 0;
    this.lineY = 0;
...
Window_QuestInfo.prototype.createQuestBitmap = function () {
    ...
    if (this.quest > 0) {
        ...
        if (this.failedImg !== "" && q.status === "failed") {
            this.questBitmap.paintOpacity = Number(
                GameusScripts["Config"]["QuestSystem"]["Failed Image Opacity"] || 128
            );
            this.questBitmap.blt(
                this.failedImg,
                0,
                0,
                this.failedImg.width,
                this.failedImg.height,
        ...
        if (this.completedImg !== "" && q.status === "completed") {
            this.questBitmap.paintOpacity = Number(
                GameusScripts["Config"]["QuestSystem"]["Completed Image Opacity"] || 128
            );
            this.questBitmap.blt(
                this.completedImg,
                0,
                0,
                this.completedImg.width,
                this.completedImg.height,
...
Window_QuestInfo.prototype.drawQuestSteps = function (q) {
    ...
    if (maxSteps === 0) maxSteps = q.steps.length;
    if (q.currentStep >= q.steps.length) q.currentStep = q.steps.length - 1;
    ...
    for (var i = 0; i < drawableSteps.length; i += 1) {
        ...
        if (step[1] === true) {
            ...
            var maxVal = step[3];
            if (step[4]) stepText += " " + String(Math.floor((varVal / maxVal) * 100)) + "%";
            else stepText += " " + String(varVal) + " / " + String(maxVal);
        }
        ...
        for (var j = 0; j < lines.length; j += 1) {
            var bulletOffset = 0;
            if (j > 0) bulletOffset += this.contents.measureTextWidth(bullet);
    ...
    for (var i = 0; i < q.rewards.length; i += 1) {
        ...
        switch (reward[0]) {
            ...
            case "gold":
                amount = reward[1];
                this.questBitmap.drawText(
                    bullet + TextManager.currencyUnit,
                    0,
                    this.lineY,
                    this.contentsWidth(),
                    this.lineHeight()
...
Window_QuestInfo.prototype.updateArrows = function () {
    this.downArrowVisible =
        this.questBitmap.height > this.contentsHeight() && this.offY < this.questBitmap.height - this.contentsHeight();
...
Window_QuestInfo.prototype.cursorDown = function (wrap) {
    if (
        this.questBitmap.height > this.contentsHeight() &&
...
Window_Quests.prototype.initialize = function () {
    var xx =
        (GameusScripts["Config"]["QuestSystem"]["Reverse Layout"] || "false").toLowerCase() === "true"
            ? Graphics.boxWidth - 320
            : 0;
    var yy =
        String(GameusScripts["Config"]["QuestSystem"]["Filter Position"]).toLowerCase() === "top"
    ...
    this.expanded = [];
    for (var i = 0; i < this.cats.length; i += 1) this.expanded[i] = false;
    this.filter = "all";
    this.refreshQuests();
...
Window_Quests.prototype.refreshQuests = function () {
    ...
    for (var i = 0; i < this.cats.length; i += 1) {
        ...
        for (var j = 0; j < this.data.length; j += 1) {
            ...
            if (q.cat == i && (this.filter == q.status || this.filter == "all")) this.counter[i] += 1;
        }
    }
};
...
Window_Quests.prototype.makeCommandList = function () {
    ...
    if (useCats === "true") {
        for (var i = 0; i < this.cats.length; i += 1) {
            flag = true;
            if (showCount === "true") count = " (" + String(this.counter[i]) + ")";
            if (catMode == 1 && this.counter[i] == 0) flag = false;
            if (catMode > 0 || this.counter[i] > 0) this.addCommand(this.cats[i] + count, "cat", flag, String(i));
            if (this.expanded[i]) {
                flag = false;
                for (var j = 0; j < this.data.length; j += 1) {
                ...
                if (!flag)
                    this.addCommand(
                        "  " + GameusScripts["Config"]["QuestSystem"]["No Quests Text"] || "No Quests",
                        "none"
                    );
            }
        }
        ...
        for (var i = 0; i < this.data.length; i += 1) {
            ...
            if (q.status == this.filter || this.filter == "all") this.addCommand(q.name, "quest", true, q.questId);
        }
    }
...
Window_QuestFilter.prototype.refresh = function () {
    if (this.filter == this.qFilters[this.filterIndex]) return;
    this.filter = this.qFilters[this.filterIndex];
    this.contents.clear();
...
Scene_Quest.prototype.update = function () {
    ...
    if (this.oldIndex != index) {
        ...
        if (q.symbol === "quest") this.questInfo.setQuest(q.ext);
        else this.questInfo.setQuest(-1);
        this.oldIndex = index;
    }
