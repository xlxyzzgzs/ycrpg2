...
(function () {
    ...
    Sprite_VirtualButton.prototype.GetPressPoint = function () {
        ...
        if (window.TouchEvent && touches instanceof TouchEvent) {
            ...
            if (
                ...
                (this == SceneManager._scene._directionalPad || this == SceneManager._scene._directionalPadPoint)
            ) {
                //Nothing to do
                //console.log("skip");
            } else {
                ...
                for (i = 0; i < touches.length; i++) {
                    ...
                    if (this.containsPoint(point) && !Alias.TouchIdUsed[touch.identifier]) {
                        if (
                            SceneManager._scene._controlButton &&
    ...
    Sprite_DirectionalPad.prototype.updateTouchInput = function () {
        ...
        var point = null;
        if (
            Alias.globalShowStates.controlMethod == "fixed rocker" ||
    ...
    Sprite_KeyButton.prototype.initialize = function (
        x,
        y,
        image,
        hotImage,
    ...
    Sprite_ControlButton.prototype.initialize = function (x, y, image, soundEffect) {
        ...
        this._buttonsHidden = !(
            Alias.globalShowStates.dPadSettings &&
        ...
        if (!!point) {
            ...
            this.x =
                point.x - this.width / 2 < 0
                    ? 0
                    : point.x - this.width / 2 > Graphics.width - this.width
                    ? Graphics.width - this.width
                    : point.x - this.width / 2;
            this.y =
                point.y - this.height / 2 < 0
                    ? 0
                    : point.y - this.height / 2 > Graphics.height - this.height
    ...
    Scene_Base.prototype.createDirPad = function () {
        ...
        if (params) {
            ...
            if (this instanceof Scene_Base) {
                ...
                this._directionalPadDynamic = new Sprite_DirectionalPadDynamic(
                    params["Rectangle X"],
                    params["Rectangle Y"],
                    params["Dynamic rocker back img"]
                );
        ...
        if (params) {
            if (params.length > 0) {
                ...
                for (var i = 0; i < params.length; i++) {
                    ...
                    if (this instanceof Scene_Base) {
                        var a = params[i];
                        this._keyButtons[a.name.toLowerCase()] = new Sprite_KeyButton(
                            a.x,
                            a.y,
                            a.image,
                            a.hotImage,
                            a.soundEffect,
                            a.inputTrigger.toLowerCase(),
                            a.customCode,
                            a.vibratePattern,
                            a.inputMethod,
                            a.commonEvent
    ...
    Scene_Base.prototype.setControlMethod = function (method) {
        ...
        switch (method) {
            ...
            case "dynamic rocker":
                ...
                if (this._directionalPadDynamic && Alias.globalShowStates.dPadSettings)
                    this._directionalPadDynamic.show();
                ...
                if (this._directionalPad && Alias.globalShowStates.dPadSettings) {
                    ...
                    this._directionalPad.x = isNaN(Parameters.dPadSettings.x)
                        ? eval(Parameters.dPadSettings.x)
                        : Parameters.dPadSettings.x;
                    this._directionalPad.y = isNaN(Parameters.dPadSettings.y)
                        ? eval(Parameters.dPadSettings.y)
                        : Parameters.dPadSettings.y;
                }
            ...
            case "fixed button":
                ...
                //if(this._directionalPadPoint && Alias.globalShowStates.dPadSettings) this._directionalPadPoint.show();
                if (this._directionalPadButton && Alias.globalShowStates.dPadSettings)
                    this._directionalPadButton.show();
                Object.keys(this._keyButtons).forEach(function (key) {
                    if (Alias.globalShowStates.keyButtonSettings[key]) SceneManager._scene._keyButtons[key].show();
                });
                break;
    ...
    if (Parameters.hideButtonsDuringDialogue) {
        ...
        Window_Message.prototype.startMessage = function () {
            ...
            if (SceneManager._scene._keyButtons["debug"]) {
                //&&Alias.globalShowStates.controlMethod!='touch input'){
    ...
    Object.defineProperties(Game_CharacterBase.prototype, {
        _moveSpeed: {
            get: function () {
                if (
                    SceneManager._scene &&
                    SceneManager._scene._directionalPadPoint &&
                    SceneManager._scene._directionalPadPoint.active
                ) {
                    ...
                    var width = Math.max(
                        SceneManager._scene._directionalPad.width,
                        SceneManager._scene._directionalPad.height
                    );
