...
DataManager.processRecipeCounts = function (obj) {
    if (obj.recipeItem.length > 0 || obj.recipeWeapon.length > 0 || obj.recipeArmor.length > 0) {
...
Yanfly.IS.Game_Interpreter_pluginCommand = Game_Interpreter.prototype.pluginCommand;
...
Yanfly.IS.Window_MenuCommand_addOriginalCommands = Window_MenuCommand.prototype.addOriginalCommands;
...
Window_SynthesisStatus.prototype.drawCollectedRecipes = function (dy) {
    ...
    var value = parseFloat($gameSystem.totalRecipes()) / Yanfly.IS.SynthesisRecipeCount;
    var text = String(parseInt(value * 100)) + "%";
    this.drawText(text, 0, dy, dw, "right");
    dw -= this.textWidth("100%") + this.standardPadding() * 2;
    var fmt = "%1/%2";
    text = fmt.format($gameSystem.totalRecipes(), Yanfly.IS.SynthesisRecipeCount);
    this.drawText(text, 0, dy, dw, "right");
...
Window_SynthesisStatus.prototype.drawCraftedItems = function (dy) {
    ...
    this.changeTextColor(this.normalColor());
    var value = parseFloat($gameSystem.synthedItems().length) / Math.max(1, Yanfly.IS.SynthesisItemTotal);
    var text = String(parseInt(value * 100)) + "%";
    this.drawText(text, 0, dy, dw, "right");
    dw -= this.textWidth("100%") + this.standardPadding() * 2;
    var fmt = "%1/%2";
    text = fmt.format($gameSystem.synthedItems().length, Yanfly.IS.SynthesisItemTotal);
    this.drawText(text, 0, dy, dw, "right");
    return dy + this.lineHeight();
};
...
Window_SynthesisStatus.prototype.drawCraftedWeapons = function (dy) {
    ...
    this.changeTextColor(this.normalColor());
    var value = parseFloat($gameSystem.synthedWeapons().length) / Math.max(1, Yanfly.IS.SynthesisWeaponTotal);
    var text = String(parseInt(value * 100)) + "%";
    this.drawText(text, 0, dy, dw, "right");
    dw -= this.textWidth("100%") + this.standardPadding() * 2;
    var fmt = "%1/%2";
    text = fmt.format($gameSystem.synthedWeapons().length, Yanfly.IS.SynthesisWeaponTotal);
    this.drawText(text, 0, dy, dw, "right");
    return dy + this.lineHeight();
};
...
Window_SynthesisStatus.prototype.drawCraftedArmors = function (dy) {
    ...
    this.changeTextColor(this.normalColor());
    var value = parseFloat($gameSystem.synthedArmors().length) / Math.max(1, Yanfly.IS.SynthesisArmorTotal);
    var text = String(parseInt(value * 100)) + "%";
    this.drawText(text, 0, dy, dw, "right");
    dw -= this.textWidth("100%") + this.standardPadding() * 2;
    var fmt = "%1/%2";
    text = fmt.format($gameSystem.synthedArmors().length, Yanfly.IS.SynthesisArmorTotal);
    this.drawText(text, 0, dy, dw, "right");
    return dy + this.lineHeight();
};
...
Window_SynthesisList.prototype.updateHelp = function () {
    ...
    } else if (eval(Yanfly.Param.ISMaskUnknown) && !$gameSystem.hasSynthed(this.item())) {
...
Yanfly.IS.Scene_Menu_createCommandWindow = Scene_Menu.prototype.createCommandWindow;
